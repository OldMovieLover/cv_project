...
# Если изображение загружено, применяем YOLO
if image is not None:
    ...
    st.image(annotated_image, caption="Результат предсказания", use_column_width=True)

    # Визуализация времени ответа
    st.write(f"Время классификации: {response_time:.4f} секунд")
else:
